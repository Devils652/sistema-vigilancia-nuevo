...
{{-- Inicializamos Alpine aquí para que controle TODO el contenido --}}
<div x-data="{ 
    showModal: false,
    isEditing: false,
    ...
    <div class="flex flex-col sm:flex-row justify-between items-end mb-8 gap-4">
        ...
        <div class="flex flex-wrap gap-3">
            <a href="{{ route($prefix . 'cameras.multiview') }}" class="inline-flex items-center gap-2 px-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 text-sm font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-700 transition-all">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M4 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2H6a2 2 0 01-2-2V6zM14 6a2 2 0 012-2h2a2 2 0 012 2v2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6zM4 16a2 2 0 012-2h2a2 2 0 01-2 2H6a2 2 0 01-2-2v-2zM14 16a2 2 0 012-2h2a2 2 0 01-2 2h-2a2 2 0 01-2-2V6z" /></svg>
                Video Wall
            </a>

            @can('crear_camaras')
                {{-- Botón corregido para abrir modal en modo CREAR --}}
                <button 
                    @click="
                        showModal = true; 
                        isEditing = false; 
                        modalTitle = 'Nuevo Grupo'; 
                        groupName = ''; 
                        actionUrl = '{{ route($prefix . 'cameras.group.store') }}';
                        method = 'POST';
                    "
                    class="inline-flex items-center gap-2 px-4 py-2.5 bg-indigo-100 dark:bg-indigo-900/30 text-indigo-700 dark:text-indigo-300 text-sm font-bold rounded-xl hover:bg-indigo-200 dark:hover:bg-indigo-900/50 transition-all border border-indigo-200 dark:border-indigo-800 cursor-pointer"
                >
    ...
    @if($groupedCameras->isNotEmpty())
        @foreach($groupedCameras as $groupId => $cameras)
            @php
                // Recuperamos el objeto del grupo o definimos "Sin Grupo"
                $group = $groupId ? ($allGroups[$groupId] ?? null) : null;
                $groupName = $group ? $group->name : 'Sin Grupo Asignado';
            @endphp
            
            <div class="relative flex py-5 items-center animate-fade-in group/header">
                <div class="flex-grow border-t border-slate-200 dark:border-slate-700"></div>
                
                <div class="flex items-center gap-3 mx-4">
                    <span class="flex-shrink-0 text-slate-400 dark:text-slate-500 text-xs uppercase tracking-widest font-bold bg-slate-50 dark:bg-slate-950 px-3 py-1 border border-slate-200 dark:border-slate-800 rounded-full">
                        {{ $groupName }}
                    </span>

                    {{-- Botones de Gestión (Solo si existe un grupo real y se tienen permisos) --}}
                    @if($group && Auth::user()->can('crear_camaras'))
                        <div class="flex items-center gap-1 opacity-0 group-hover/header:opacity-100 transition-opacity duration-200">
                            {{-- Editar --}}
                            <button 
                                @click="
                                    showModal = true; 
                                    ...
                                    actionUrl = '{{ route($prefix . 'cameras.group.update', $group->id) }}';
                                    method = 'PUT';
                                "
                            ...
                            {{-- Eliminar --}}
                            <form action="{{ route($prefix . 'cameras.group.destroy', $group->id) }}" method="POST" onsubmit="return confirm('¿Borrar este grupo? Las cámaras quedarán sin asignar.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors" title="Borrar grupo">
        ...
        <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div x-show="showModal" x-transition:enter="ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" x-transition:enter-end="opacity-100 translate-y-0 sm:scale-100" x-transition:leave="ease-in duration-200" x-transition:leave-start="opacity-100 translate-y-0 sm:scale-100" x-transition:leave-end="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" class="inline-block align-bottom bg-white dark:bg-slate-800 rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full border border-slate-200 dark:border-slate-700">
                ...
                <form :action="actionUrl" method="POST">
                    @csrf
                    {{-- Simulación del método PUT si estamos editando --}}
                    ...
                    <div class="px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
                        <div class="sm:flex sm:items-start">
                            ...
                            <div class="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left w-full">
                                <h3 class="text-lg leading-6 font-bold text-slate-900 dark:text-white" x-text="modalTitle">
                                    </h3>
                                <div class="mt-2">
                                    <input type="text" name="name" required x-model="groupName"
                    ...
                    <div class="bg-slate-50 dark:bg-slate-700/30 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-2">
                        ...
                        <button @click="showModal = false" type="button" class="mt-3 w-full inline-flex justify-center rounded-xl border border-slate-300 dark:border-slate-600 shadow-sm px-4 py-2 bg-white dark:bg-slate-800 text-base font-medium text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 focus:outline-none sm:mt-0 sm:w-auto sm:text-sm">
                            Cancelar
                        </button>
                    </div>
                </form>
